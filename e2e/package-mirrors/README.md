...
The Homebrew cold phase covers all three protocol surfaces explicitly: it first asks native Homebrew for API/JWS metadata through `HOMEBREW_API_DOMAIN`, probes the mirrored Homebrew/core Git HEAD, and then enables `HOMEBREW_NO_INSTALL_FROM_API=1` before installing the standard Bottle through Katch. The warm phase skips the mutable API and Git checks, enables the same tap mode, and installs the same formula/Bottle identity from the image-pinned core tap. This is deliberate: API/JWS and Git remain governed by their honest HTTP and Git freshness rules, while the immutable Bottle can be reused with zero origin requests. API cache, tap state, package state, `HOME`, and the Cellar remain phase-local rather than being shared to manufacture a warm hit. The pinned client validates cold API and Tap evidence, the native installed-formula list and JSON, and the Bottle install receipt without hardcoding the current jq version. It deliberately does not execute the formula binary because upstream bottles may advance their minimum glibc beyond the pinned image's OS ABI. The shared regression case always runs Git. Docker and Podman remain explicitly blocked because no daemon is present in the restricted client namespace; a host daemon would bypass the firewall.
