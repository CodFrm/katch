...
Debian's `InRelease` carries `cache-control: public, max-age=120`, and a CDN `Age` can consume most of that lifetime before the index even arrives, so a warm `apt-get update` is entitled to revalidate and a zero-origin warm phase cannot be promised for it. The APT warm phase therefore does not refresh indexes: the cold phase verifies `InRelease` with `gpgv` and only then publishes that index and its package lists under `$KATCH_SHARED/apt-lists`, the warm phase restores them into an otherwise empty `/var/lib/apt/lists`, verifies the signature again, and installs `hello` so that only the immutable `.deb` crosses the network.

The warm claim rests on the following, and each part can fail. Both phases start from a container where `hello` is not installed and require apt's own `Get:` line for it, because neither dpkg's state (it cannot distinguish a fresh install from a package the image already carried) nor the archive cache (`docker-clean` deletes the `.deb` as soon as dpkg is done) can show that bytes moved. These checks are written as `if …; then exit 1; fi`, never as `!`-negated pipelines, which `set -e` ignores unless they are a script's last command, and apt writes to a log file instead of through `tee`, whose exit status would mask a failed install. The warm phase compares the index it verified and installed from against the published copy byte for byte. No `.deb` may appear anywhere under `$KATCH_SHARED`, so the warm download cannot be a reused local archive. That the warm phase does not refresh indexes at all is a property of its run script, pinned by the self-test's check that `apt-get` appears exactly twice (one cold update, one install); no runtime file state can show it, because apt stamps each list with the origin's `Last-Modified`, so a refetch of an unchanged `InRelease` leaves both its bytes and its timestamp exactly as the restore did. A refresh would in any case either reach the origin, failing the zero-warm check, or be answered from Katch's cache.
