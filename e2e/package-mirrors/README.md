...
Three assertions carry that claim, and each one can fail. Both phases start from a container where `hello` is not installed and require apt's own `Get:` line for it, because neither dpkg's state (it cannot distinguish a fresh install from a package the image already carried) nor the archive cache (`docker-clean` deletes the `.deb` as soon as dpkg is done) can show that bytes moved. The warm phase compares the index it verified against the published copy and requires the two timestamps to match: byte equality alone would not rule out a refresh, since the origin may hand back the same `InRelease`, but a refetch rewrites the file while the restore kept the cold phase's timestamp. And no `.deb` may appear anywhere under `$KATCH_SHARED`, so the warm download cannot be a reused local archive.
